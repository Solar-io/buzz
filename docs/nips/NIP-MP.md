...
**Mode 1 — composite cursor (exhaustive under the relay contract).** On a relay that exposes a keyset cursor over `(created_at, event id)`, a client MUST page by it. As an example of the cursor mechanics, Buzz implements the keyset as `created_at < until OR (created_at = until AND id > before_id)` (`crates/buzz-db/src/event.rs:48-52`), resolving the sort to `(created_at DESC, id ASC)`. Buzz exposes this cursor on both of its read transports: the authenticated HTTP bridge endpoint (`crates/buzz-relay/src/api/bridge.rs`) and the NIP-01 websocket REQ path. `before_id` is an extension field on the filter object, and `nostr::Filter`'s deserializer drops unknown fields, so on either transport the relay reads it from the raw filter JSON before that conversion (`crates/buzz-relay/src/cursor.rs`, shared by both). The cursor grammar is the same on both: `before_id` requires `until`, and a `before_id` that is not a 64-character hex event id is refused — a `400` on HTTP, a NIP-01 `CLOSED` on the websocket — never silently demoted to a half cursor or a head request. A client that sends a cursor Buzz cannot honour therefore learns so, which is what makes mode selection decidable; a relay that discards the field instead is indistinguishable from one that applied it. Mode selection still requires evaluating the relay contract per transport, because that is a property of the relay and not of NIP-01. Within the relay contract, the uniqueness of the `(created_at, id)` pair means each page resumes exactly where the last ended with no skips or re-reads, and a short page is an unambiguous end signal. Cursor uniqueness adds tie-safety; it does not substitute for the relay contract — a relay that post-filters after limiting can return an empty page under this cursor while older matching events remain beyond the candidate window.
...
**Query shapes.** The relay contract applies only where the relay can apply it — and that depends on the query shape. A relay that post-filters some constraints — a tag match applied after the SQL `LIMIT`, say — cannot guarantee short-response exhaustion for queries that use those constraints. A client MUST therefore issue fold queries in shapes whose full filter the relay applies before limiting. Where a needed constraint is not applied pre-limit on the target relay, the client MUST widen the query to constraints that are — for example, enumerating all `kind:5` events by `kinds` alone, or `kinds` + `authors`, rather than adding a tag filter the relay post-applies — and match the remaining criteria client-side. This keeps the relay contract's short-response guarantee intact for every query the fold issues.

Buzz applies `#a` before its `LIMIT`, pushing the coordinate match into SQL alongside `kinds`, `authors`, `ids` and `#e` (`EventQuery::a_tags`, `crates/buzz-db/src/event.rs`), so a `#a`-scoped fold query on Buzz satisfies condition 1 above. That is a property of this relay and not of NIP-01: a client MUST still evaluate the contract per relay rather than assume a pre-limit `#a` anywhere it sees one.
