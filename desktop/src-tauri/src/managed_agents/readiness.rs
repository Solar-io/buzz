...
fn buzz_agent_requirements(effective: &EffectiveAgentEnv) -> Vec<Requirement> {
    ...
    // Provider-specific credential requirements, from the shared table in
    // `credentials` (also read by usage-attribution seeding, which uses the
    // key's *name* as an observed-configuration signal and never its value).
    // A key present with an empty value is treated as absent — matching the
    // dialog's (envVars[key] ?? "").length === 0 emptiness check. An unknown
    // provider, or none yet, has no credential requirement: only the
    // NormalizedField requirement above captures that gap.
    let env_key_missing = |key: &str| effective.env.get(key).is_none_or(|v| v.is_empty());
    if let Some(key) = credentials::provider_credential_key(provider) {
...
fn goose_requirements(
    ...
    if let Some(key) = credentials::provider_credential_key(effective_provider) {
