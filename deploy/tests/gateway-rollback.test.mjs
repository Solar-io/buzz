...
function fixture(options = {}) {
  ...
  writeFileSync(stateFile, JSON.stringify({ containers: [old, { ...old, Id: "e".repeat(64), Name: "/buzz-dev-postgres-1" }], ...options }));
  const backup = join(dir, "dump"); writeFileSync(backup, "verified fixture db dump");
  const manifest = join(dir, "backup.json"); writeFileSync(manifest, JSON.stringify({ version: 1, verified: true, containers: { gateway: old.Id }, artifacts: [{ path: backup, sha256: createHash("sha256").update(readFileSync(backup)).digest("hex") }] }));
  const registry = join(dir, "ports.json"); writeFileSync(registry, JSON.stringify({ project_port_blocks: { buzz: { push_gateway: 61359, push_gateway_metrics: 61362 } } }));
  return { dir, stateFile, run(extra = []) {
    return spawnSync("bash", [resolve(root, "deploy/push-gateway-up.sh"), "--replace-native", "--image", "fixture-image", "--delivery-url", "https://fixture.tailnet.test/v1/deliveries/apns", "--backup-manifest", manifest, "--state-dir", join(dir, "deployment"), "--execute", ...extra], { cwd: root, encoding: "utf8", timeout: 15000, env: { ...process.env, PATH: `${bin}:${process.env.PATH}`, CAP_FIXTURE_STATE: stateFile, PORT_REGISTRY: registry, CAP_DEPLOY_LOG: join(dir, "verification.log"), CAP_HEALTH_RETRIES: "1", CAP_HEALTH_DELAY: "0" } });
