...
| Web chat client | Phase 0 commit(s), `feat(web): Phase 0 web client…` (branch `claude/web-front`, growing series) | Additive in upstream's own `web/` package: encrypted local identity (AES-GCM/PBKDF2 envelope in IndexedDB), persistent relay session (NIP-42 AUTH + reconnect replay), login/unlock/QR device pairing, responsive shell + live channel list, PWA; `/repos` renders `ReposPage` directly (upstream redirected to `/`, which is now the auth-gated shell — the only behavioral change to existing code); `deploy/compose/compose.web.yml` overlay + launchd `compose.web.yml` layer serve the bundle from the relay origin via `BUZZ_WEB_DIR` | Human-facing web/iOS/iPadOS client (Sam, 2026-08-29: DMs, channels, agent admin, huddle-voice, file upload) — the relay is already web-first; the client didn't exist | **Upstream-PR candidate per-phase** — the auth/store/session modules are generic; the product direction (chat UI) tracks upstream's own `web/` package, submit after it stabilizes past Phase 1 |
