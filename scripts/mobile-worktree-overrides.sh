...
# Append the slug to the prefix that is actually in effect (an internal
# build may have renamed it in AppOverrides.xcconfig) rather than to this
# file's idea of the default. The generated xcconfig keeps the reference
# unresolved so Xcode picks up the prefix from the include chain; the
# resolved value is only used for the human-readable summary below.
ios_bundle_id_prefix="$("$repo_root/scripts/mobile-bundle-id-prefix.sh")"
ios_bundle_id="${ios_bundle_id_prefix}.${ios_slug}"
...
BUNDLE_IDENTIFIER = \$(BUNDLE_ID_PREFIX).${ios_slug}
