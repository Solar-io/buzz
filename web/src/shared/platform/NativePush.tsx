...
function useNativePushService() {
  ...
  const [profile, setProfile] = useState<
    "buzz-capacitor-ios-sandbox" | "buzz-capacitor-ios-production" | null
  >(null);
  useEffect(() => {
    if (!isNativeIOS()) return;
    void BuzzPush.isSupported()
      .then((result) => {
        if (
          result.appProfile === "buzz-capacitor-ios-sandbox" ||
          result.appProfile === "buzz-capacitor-ios-production"
        )
          setProfile(result.appProfile);
      })
      .catch(() => {});
  }, []);
  return useMemo(() => {
    ...
    const wake = async () => {
      ...
      try {
        ...
        if (result.status === 404) {
          ...
          if (!result.ok)
            throw new Error(
              "Could not open this notification. Retry when connected.",
            );
          ...
          await router.navigate({
            ...
            search: value.channel_id
              ? { c: value.channel_id, m: value.event_id }
              : { view: "inbox" },
          });
        }
  ...
  return (
    <section className="space-y-3 rounded-lg border border-border bg-card p-4">
      ...
      <p className="text-sm text-muted-foreground">
        Receive mentions while Buzz is closed. Notifications contain no message
        text; opening one retrieves it from your relay.
      </p>
      {!service && (
        <p className="text-sm">
          Configure your push gateway and sign in first.
        </p>
      )}
      {error && (
      ...
      <button
        ...
        onClick={() => {
          ...
          setBusy(true);
          setError("");
          void (status?.enabled ? service.disable() : service.enable())
            .then(setStatus)
      ...
      {status?.pending && (
        <p className="text-sm">
          Registration is waiting for relay acknowledgment.
        </p>
      )}
    </section>
  );
