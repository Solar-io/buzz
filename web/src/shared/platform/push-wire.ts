...
export type PushSubscription = {
  ...
  class: "default" | "silent" | "time_sensitive";
};

export function enrollTranscript(
  c: GatewayChallenge,
  keyId: string,
  profile: string,
  token: string,
...
export function rotateTranscript(
  c: GatewayChallenge,
  handle: string,
  epoch: number,
  token: string,
...
export function revokeTranscript(
  c: GatewayChallenge,
  handle: string,
  relay: string,
  generation: number,
...
export function renewTranscript(
  c: GatewayChallenge,
  handle: string,
  epoch: number,
  expires: number,
): string {
  return `buzz.push.renew-installation.v1\n${JSON.stringify({ v: 1, audience: "https://push.buzz.xyz/v1/installations/renew", challenge_id: c.challenge_id, challenge: c.challenge, installation_handle: handle, endpoint_epoch: epoch, expires_at: expires })}`;
}
