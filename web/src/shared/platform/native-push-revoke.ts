...
export async function revokeNativePush(): Promise<void> {
  ...
  if (
    appProfile !== "buzz-capacitor-ios-sandbox" &&
    appProfile !== "buzz-capacitor-ios-production"
  )
    throw new Error(
      "Cannot revoke push: native installation profile unavailable.",
    );
  const service = createPushEnrollment({
    relayUrl: services.relayUrl,
    gatewayUrl: services.pushGatewayUrl,
    profile: appProfile,
    ...
    publish: async (event) => {
      ...
      if (!response.ok)
        throw new Error(
          "Push revocation failed. Your identity has been retained; reconnect and try again.",
        );
