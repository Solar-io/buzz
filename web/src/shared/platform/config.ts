...
export interface NativeServices { relayUrl: string; sttUrl: string; ttsUrl: string; pushGatewayUrl: string }
...
export function validateServices(input: NativeServices): NativeServices {
  for (const [name, value] of Object.entries(input)) {
    ...
    if (url.protocol !== scheme || !url.hostname || url.username || url.password || url.hash || url.search) {
...
export function readNativeServices(): NativeServices | null {
  try {
    ...
    return validateServices({ relayUrl: parsed.relayUrl, sttUrl: parsed.sttUrl ?? "", ttsUrl: parsed.ttsUrl ?? "", pushGatewayUrl: parsed.pushGatewayUrl ?? "" });
  } catch { return null; }
}
