...
// Ported from the desktop client's shared/ui/popover.tsx. Deliberate divergence:
// desktop's `surface="textured"` variant is not ported to web, so the `default`
// surface is the only one and `sideOffset` defaults to 4 unconditionally.
