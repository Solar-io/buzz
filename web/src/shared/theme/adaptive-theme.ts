 ...
 *
 * ---
 * WEB COPY. Byte-for-byte copy of `desktop/src/shared/theme/adaptive-theme.ts`
 * as of 2026-09-03. The web client is deliberately decoupled from the desktop
 * package — there is no shared workspace package and no cross-package import,
 * so this file is *owned here* and may diverge. The desktop is the reference,
 * not the source of truth.
 *
 * `adaptive-theme.test.mjs` pins the behavior with vectors rather than
 * diffing against the desktop: if the desktop changes, this copy keeps
 * working and following it becomes an explicit decision, not an accident.
 * Update that test in the same change if you ever alter the math here.
 */
