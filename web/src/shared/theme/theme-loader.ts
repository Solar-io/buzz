 ...
 *
 * ---
 * WEB COPY of `desktop/src/shared/theme/theme-loader.ts` as of 2026-09-03,
 * with one deliberate divergence: the terminal palette is dropped. The web
 * client has no terminal surface, so `extractTerminalPalette` and the
 * `terminalPalette` field on {@link ThemeInfo} are omitted rather than
 * carried as dead weight. Everything else — the theme registry, the
 * light/dark sets, the pairings and the colour extraction — is unchanged.
 *
 * The web client is deliberately decoupled from the desktop package: no
 * shared workspace package, no cross-package import. This file is owned here
 * and may diverge further; the desktop is the reference, not the source.
 */
