 ...
 * custom `<spoiler>` element, rendered by MarkdownContent.
 *
 * ---
 * WEB COPY of `desktop/src/shared/lib/remarkSpoilers.ts` as of 2026-09-03,
 * unchanged. It has no imports, so it moves verbatim. The web client is
 * deliberately decoupled — no cross-package imports — so this file is owned
 * here and may diverge; the desktop is the reference, not the source.
 *
 * This exists because the composer can now WRITE `||spoiler||` (the wire
 * syntax desktop and mobile parse), and a client that emits syntax it cannot
 * itself render ships a button that looks broken to its own author.
 */
