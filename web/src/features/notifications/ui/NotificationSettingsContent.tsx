...
export interface NotificationSettingsContentProps {
  /**
   * Renders the closing button when provided (the dialog passes its close
   * handler; the inline settings pane has nothing to close and omits it).
   */
  onDone?: () => void;
}

/**
 * The notification controls themselves, shared by the dialog
 * (`NotificationSettingsDialog`) and the settings page's Notifications pane.
 *
 * The permission gesture survives the move: the browser only grants
 * notification permission from a real user gesture, and the only call to
 * `promptForNotificationPermission` sits inside the desktop-notifications
 * switch's `onCheckedChange` — the event handler of the control the user
 * actually clicked. Nothing here prompts on mount, so embedding the content
 * in a pane is as safe as it was behind the dialog's "Manage" button.
 */
  ...
  return (
    <div className="flex flex-col gap-5 pt-1">
      ...
      <fieldset className="flex flex-col gap-2">
        <legend className="pb-2 text-sm font-medium">What notifies you</legend>
      ...
      <section className="flex items-center justify-between gap-4">
        <div className="min-w-0">
          ...
          <p className="text-2xs text-muted-foreground">
            Shows a count in the browser tab while Buzz is in the background.
          </p>
        </div>
        <Switch
...
/**
 * The Notifications pane body: the dialog's controls inlined into a card,
 * with the same "this browser only" framing the dialog opens with.
 */
