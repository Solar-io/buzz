...
import { useAgentMemory } from "@/features/agent-memory/hooks";
import type { MemoryTreeNode } from "@/features/agent-memory/lib/buildMemoryGraph";
import type { EngramEntry } from "@/features/agent-memory/lib/engrams";
...
const MEMORY_UNDECRYPTABLE_TOOLTIP =
  "These memory records did not open with your key — most likely they were written for a different owner.";

/**
 * Memory section — read-only NIP-AE viewer, ported from the desktop client's
 * `features/agent-memory/ui/MemorySection.tsx`.
 *
 * OWNER GATING IS A UX GATE, NOT A SECURITY BOUNDARY. The caller passes
 * `viewerIsOwner` and we render nothing when it is false, mirroring the
 * desktop's `isCurrentUserOwner || isOwner`. What actually protects an
 * agent's memory is (a) NIP-44 encryption to the OWNER's pubkey — only the
 * owner's secret key opens it — and (b) the relay refusing an engram REQ
 * whose `#p` is not the authenticated reader (`engram_filters_authorized`,
 * crates/buzz-relay/src/handlers/req.rs). A non-owner who bypassed this flag
 * would get an empty result from the relay and undecryptable bytes if they
 * somehow did not. The flag only decides whether we bother asking.
 *
 * The whole thing is contained: skeleton/error/empty live INSIDE this
 * section so the rest of the panel stays interactive while we fetch and
 * decrypt. Refetch is non-blocking — cached data stays on screen while
 * `isFetching` is true.
 *
 * Layout:
 *   ⚠ stale-refetch banner (if a refresh failed with data on screen)
 *   ── entries: `core` first, then its reachable descendants, then orphans ──
 *   ── "View all (n)" with the relay-truncation tooltip ──
 *   ── per-entry missing-`[[slug]]` hints ──
 *   ⚠ undecryptable-record count (web-specific: decryption happens here)
 */
...
export function MemoryRefreshButton({
  ...
  const { query } = useAgentMemory(agentPubkey, { enabled: viewerIsOwner });
...
function MemorySectionForOwner({
  ...
  const { query, graph } = useAgentMemory(agentPubkey);

  // Order matters here. We want:
  // - first paint, no cache → skeleton
  // - error with no cache → error state (with retry)
  // - error WITH cache → keep the data, show a non-blocking "refresh failed"
  //   banner; the user can retry without losing what they had
  // - data, but empty → empty state
  // - data, non-empty → render
  //
  // `isPending && !isError` also covers the pre-connection window: the query
  // is disabled until the relay session reports `open`, and a bare spinner-
  // less gap there would read as "this agent has no memories".
  const showInitialSkeleton =
    (query.isLoading || query.isPending) && !query.data && !query.isError;
      ...
      {query.data && graph ? (
        <>
          ...
          <MemoryGraphView
            ...
            undecryptable={query.data.undecryptable}
            variant={variant}
          />
        </>
      ) : null}
...
function MemoryStaleErrorBanner({
  ...
  return (
    <div
      className={cn(
        "mb-2 flex items-center gap-2 rounded-md border border-warning/30 bg-warning-bg px-2 py-1.5 text-xs",
...
function MemoryGraphView({
  ...
  graph: NonNullable<ReturnType<typeof useAgentMemory>["graph"]>;
  truncated: boolean;
  ...
  if (isEmpty) {
    return (
      <div className="space-y-3">
        ...
        <MemoryUndecryptableHint count={undecryptable} variant={variant} />
      </div>
    );
  }
...
/**
 * Web-specific: the desktop decrypts in Rust and silently drops frames it
 * cannot open. Here the decrypt happens in the browser, and the most likely
 * cause of a failure is the viewer holding a different key than the one the
 * agent sealed to — which would otherwise render as "no memories". Count it
 * and say so.
 */
function MemoryUndecryptableHint({
  count,
  variant,
}: {
  count: number;
    ...
    <Tooltip>
      <TooltipTrigger asChild>
        <p
          ...
          data-testid="agent-memory-undecryptable"
        >
          <AlertTriangle className="h-4 w-4 shrink-0 text-warning" />
          {count} record{count === 1 ? "" : "s"} could not be decrypted.
        </p>
      </TooltipTrigger>
      <TooltipContent className="max-w-xs text-xs" side="top">
        {MEMORY_UNDECRYPTABLE_TOOLTIP}
      </TooltipContent>
    </Tooltip>
  );
}
  ...
  return (
    <Tooltip>
      <TooltipTrigger asChild>
        <div
          className={cn(
            "flex justify-center border-warning/30 bg-warning-bg px-4 py-2",
...
function MemoryBodyText({ body }: { body: string }) {
  ...
  for (const match of body.matchAll(MEMORY_REF_PATTERN)) {
    ...
    parts.push(
      <span className="break-all text-foreground" key={`${index}-${slug}`}>
        [[{slug}]]
      </span>,
    );
        ...
        <p className="text-warning">
          ...
          {slugs.map((slug, index) => (
            <React.Fragment key={slug}>
              {index > 0 ? ", " : null}
              <span
                className="break-all text-warning"
