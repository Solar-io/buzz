import type { AgentMemoryListing, EngramEntry } from "./engrams.ts";
 ...
 *
 * Ported verbatim from the desktop client's
 * `features/agent-memory/lib/buildMemoryGraph.ts` — the function is pure, so
 * only the `EngramEntry` import source differs (web decrypts in the browser;
 * the desktop receives the same shape from a Tauri command).
 *
 * Why a single pass:
 ...
 *
 * Tombstones never appear here — they're filtered out by the decode layer
 * (`decodeEngramListing`) before this function ever sees them. A `[[slug]]`
 * ref to a tombstoned memory therefore shows up as a dangling ref, which is
 * the right user-facing signal ("this memory used to exist but doesn't
 * anymore").
 */
