...
export function useNativeHuddleCall({
  target,
  selfPubkey,
}: {
  ...
  const [voiceInputMode, setVoiceInputMode] = useState<"open" | "push_to_talk">(
    "open",
  );
  ...
  const parentChannelId =
    state.parentChannelId ?? target?.parentChannelId ?? null;
  const connected =
    state.status === "connected" || state.status === "reconnecting";
  const current = useMemo<ChannelSummary | null>(
    () =>
      channelId
        ? {
            id: channelId,
  ...
  const roster = useHuddleAgentRoster({
  ...
  const profiles = useProfiles(
    useMemo(
      () => [...state.peers.map((p) => p.pubkey), ...roster.agentPubkeys],
      [state.peers, roster.agentPubkeys],
    ),
  );
  const reactions = useHuddleReactions({
    channelId: connected ? channelId : null,
    selfPubkey,
    senderName: selfPubkey
      ? (profiles.get(selfPubkey)?.displayName ?? "You")
      : "You",
  });
  ...
  useEffect(() => {
    let alive = true;
    const listener = BuzzHuddle.addListener("state", (next) => {
      if (alive) setState(next);
    });
    void BuzzHuddle.snapshot().then((next) => {
      if (alive) setState(next);
    });
    const refresh = () => {
      if (!document.hidden)
  ...
  useEffect(() => {
    if (parentChannelId)
      setPrefsState(loadHuddlePrefs(localStorage, parentChannelId));
  }, [parentChannelId]);
  const configure = useCallback(
    (options: Parameters<typeof BuzzHuddle.configure>[0]) => {
      void BuzzHuddle.configure(options).catch((error: unknown) =>
        toast.error(
          error instanceof Error
            ? error.message
            : "Could not change call settings.",
        ),
      );
    },
    [],
  ...
  useEffect(() => {
    if (connected)
      configure({ duplex: prefs.duplex, voiceOverride: prefs.voice ?? {} });
  ...
  const join = useCallback(async () => {
    ...
    try {
      ...
      setState(
        await BuzzHuddle.join({
          relayUrl: relayWsUrl(),
          channelId: target.huddleChannelId,
          parentChannelId: target.parentChannelId,
          sttUrl: services?.sttUrl ?? "",
          ttsUrl: services?.ttsUrl ?? "",
        }),
      );
    } catch (error) {
      setState((old) => ({
        ...
        status: "error",
        error:
          error instanceof Error ? error.message : "Could not join huddle.",
      }));
    }
  }, [target]);
  const leave = useCallback(() => {
    void BuzzHuddle.leave().catch((error: unknown) =>
      toast.error(String(error)),
    );
  }, []);
  const held =
    (prefs.duplex === "half" && state.speaking) ||
    (voiceInputMode === "push_to_talk" && !pttActive);
  ...
  return {
    ...
    setPrefs,
    duplex: {
      ...initialDuplexState(prefs.duplex),
      agentSpeaking: state.speaking,
      userMuted: state.muted,
      held,
    },
    ...
    huddle: {
      ...
      error: state.error,
      peers: state.peers,
      speaking: new Map(Object.entries(state.levels ?? {})),
      muted: state.muted,
      micLevel: state.micLevel ?? -127,
      devices: [],
      deviceId: "",
      outputDevices: [
        { deviceId: "speaker", label: "Speaker" },
        { deviceId: "receiver", label: "Receiver / connected headphones" },
      ],
      outputDeviceId: state.speaker ? "speaker" : "receiver",
      speakerMuted: state.speakerMuted ?? false,
      supportsOutputSelection: true,
      held,
      voiceInputMode,
      pttActive,
      micLive: connected && !held && !state.muted,
      supportsVoice: true,
      join,
      leave,
      toggleMute: () => configure({ muted: !state.muted }),
      toggleSpeakerMuted: () =>
        configure({ speakerMuted: !state.speakerMuted }),
      selectDevice: async () => {
        throw new Error("Choose the microphone in iOS audio routing.");
      },
      selectOutputDevice: async (device) => {
        await BuzzHuddle.configure({ speaker: device === "speaker" });
        return true;
      },
      setHeld: (held) => configure({ held }),
      setVoiceInputMode: (mode) => {
        setVoiceInputMode(mode);
        configure({ held: mode === "push_to_talk" && !pttActive });
      },
      setPushToTalkActive: (active) => {
        setPttActive(active);
        configure({ held: voiceInputMode === "push_to_talk" && !active });
      },
      subscribeMicFrames: () => {
        throw new Error(
          "Native microphone frames stay inside the native call engine.",
        );
      },
      resumeAudio: async () => {
        await BuzzHuddle.snapshot();
      },
    },
    voice: {
      supported: true,
      ...
      setEnabled: (enabled) =>
        configure({
          ...
          ...(enabled ? { speechEnabled: true } : {}),
        }),
      offReason: state.voiceOffReason ?? null,
      status: state.voiceStatus ?? "idle",
      interimText: state.interim,
      error: state.error,
    },
    speech: {
      supported: true,
      enabled: state.speechEnabled,
      setEnabled: (enabled) => configure({ speechEnabled: enabled }),
      agentPubkeys: new Set(roster.agentPubkeys),
      membershipKnown: members.known,
      suppressedAgents: state.peers.map((p) => p.pubkey),
