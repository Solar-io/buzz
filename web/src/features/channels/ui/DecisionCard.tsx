...
export function DecisionCard({
   ...
   * is a consequence of PUBLISHING, which is this component's job.
   */
  ...
  async function submit(current: CardInterviewState) {
    ...
    try {
      ...
      if (result.answer?.done === true) {
        writer.cancel();
        void clearCardDraft(cardId);
        // A completed interview is terminal, so the phone sheet has nothing
        // left to show. (The terminal branch unmounts it anyway; closing it
        // explicitly is what makes Radix release the body scroll-lock through
        // its own close path rather than through an unmount.)
        setSheetOpen(false);
        setPhase({
          kind: "sent",
