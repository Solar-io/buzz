...
| Workflows: list, run history + step trace, manual trigger, approvals | Stock | kind:30620 definitions plus the relay's `/workflows/{id}/runs` and `.../runs/{run}/approvals` NIP-98 reads — both upstream, neither in FORK_MANIFEST.md. Runs are read from the DB-backed endpoints, which stay authoritative; since WF-08 the relay *also* publishes kinds 46001-46012 as it goes, so a subscriber can follow a run without polling. Approvals are live: the executor's gate mints a `workflow_approvals` row and announces it as kind:46010 carrying the `approval_ref` in an `approval` tag, and the web's grant/deny writes that same ref back as the `d` tag of a kind:46030/46031. The web still reads approvals over HTTP rather than from the event, because status (granted/denied/expired) lives on the row and the event only announces the request. Editing is deliberately read-only (Sam, 2026-09-04); the web has no YAML writer, so `lib/yaml.ts` reads the schema's subset in-tree. |
...
| Moderation queue: reports, audit, per-channel authority | Stock | Reads are `GET /moderation/reports` and `/moderation/audit`, gated on `relay_members` role. **A NIP-OA owner attestation does not confer moderation authority** — it admits the key, and the relay reads the NIP-98 *signer* for role. That is deliberate and stays (Sam, 2026-09-04): NIP-OA calls the tag "authorization evidence only" and forbids reinterpreting it as an identity override; 41 agent keys are attested under the owner key, and `docs/remote-agents.md` hands agent nsecs to provider binaries by design. A web session signing as an attested agent key therefore needs its own `relay_members` row (`buzz-admin add-member --role admin`). Acting on a report follows the relay, not desktop: nothing in relay production code routes 9005/9001 through `authorize_moderation_action`, so a community owner with no role in the reported channel cannot delete or kick there. |
