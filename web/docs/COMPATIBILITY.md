...
| Identity archive | Stock | 9035/9036 requests, relay-signed 13535 snapshot with fail-open on every error path, NIP-OA owner gate with real BIP-340 verification, self-exempt predicate. **A NIP-OA attestation is authorization evidence only and never confers role** — it admits the key, and the relay reads the NIP-98 *signer* for role. That is deliberate and stays (Sam, 2026-09-04): NIP-OA forbids reinterpreting the tag as an identity override, agent keys are attested under the owner key, and `docs/remote-agents.md` hands agent nsecs to provider binaries by design — so a web session signing as an attested agent key still needs its own `relay_members` row (`buzz-admin add-member`). |
