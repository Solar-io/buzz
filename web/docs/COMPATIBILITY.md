...
| Workflows: list, run history + step trace, manual trigger, approvals | Stock | kind:30620 definitions plus the relay's `/workflows/{id}/runs` and `.../runs/{run}/approvals` NIP-98 reads. **Approvals are live as of 2026-09-04** — they were not merely inert before: `RequestApproval` discarded the token and the run was marked Failed with `approval_not_supported`, so a gated workflow died rather than waiting. The relay now mints a record, publishes kind:46010, and parks the run in `waiting_approval`; grant/deny (46030/46031) resume or cancel it, and kinds 46001-46012 are emitted as runs progress. The wire reference is the hex of the stored token hash, never the token. Editing is deliberately read-only (Sam, 2026-09-04); the web has no YAML writer, so `lib/yaml.ts` reads the schema's subset in-tree. |
