...
7. **`cargo test -p buzz-acp` from a Buzz ACP-managed shell leaks 3 false failures** — the harness injects `BUZZ_ACP_LAZY_POOL` / `BUZZ_ACP_IDLE_POOL_SLEEP` / `BUZZ_ACP_SESSION_ID` into every managed agent's env, and three pool-config tests fail on those values (panic at `config.rs:2287`/`config.rs:2294`). They are env artifacts, not regressions: unset the three vars (`env -u BUZZ_ACP_LAZY_POOL -u BUZZ_ACP_IDLE_POOL_SLEEP -u BUZZ_ACP_SESSION_ID cargo test -p buzz-acp`) and they pass. Proven twice (2026-09-12 QA); verify by rerunning the same tests on the parent commit before believing a failure is real.
8. **Web scroll-follow: never trust React `onScrollCapture` or virtua handle metrics at event time** — both measured live during the auto-tail fix (2026-09-13): React never fires `onScrollCapture` for descendant `scroll` events (scroll doesn't bubble and React neither delegates nor direct-attaches capture listeners for it), and virtua's `VListHandle.scrollOffset`/`scrollSize` can still hold pre-event values inside the scroll callback (a reader 275px up was yanked to the bottom from stale at-bottom math). The working pattern, in `ChannelTimeline.tsx` / `AgentActivityPanel.tsx`: a plain `addEventListener('scroll', h, {capture: true, passive: true})` on a wrapper ref, reading `event.target`'s own `scrollTop`/`scrollHeight`/`clientHeight`; pause on ANY upward scroll (direction outranks the at-bottom tolerance — position-only lets a fast tail erase small upward deltas), resume only back at the very bottom (`lib/scrollFollow.ts`).
9. **Mutation-testing Rust: `mv file.bak file` restores the BYTES but not the MTIME, so cargo reuses the mutant** — earned on the Phase 6 card gate (2026-09-20). `sed -i.bak` writes the mutant with a fresh mtime and leaves the ORIGINAL in `.bak` with its old one; moving it back gives cargo a file that looks older than the artifact, so the next `cargo test` reports `Finished in 0.19s` and runs the previous binary. Here it failed loudly (a reverted mutation kept its test red), but the same trap in the other direction is a false PASS: mutate, get an unchanged green, conclude the test cannot fail. `touch` the file after any restore, or revert with `git checkout --` (which does update the mtime). Node/TS suites are immune — no build cache.
10. **`infer` reports legacy MIME aliases — allow-lists must canonicalize** — `infer` 0.19 returns `audio/x-wav` for RIFF/WAVE and `audio/x-flac` for FLAC (its `map.rs`), not the canonical spellings. Any exact-match allow-list on `infer::get()` output silently rejects those formats — the 2026-09-17 `buzz upload file` WAV bug: the relay accepted both aliases and stored canonical `audio/wav`, the CLI allow-list carried only the canonical form, so every WAV died client-side while relay logs stayed clean. Canonicalize the sniffed mime before any membership test; the canonicalizers are `buzz-cli`'s `canonical_upload_mime()` and `buzz-media`'s sniff path — don't add a third. When bumping `infer`, re-read its map for NEW aliases intersecting `ALLOWED_MIMES` (the pinned 0.19.0 has exactly the two above, and only `audio/x-wav` intersects).
...
- **A card replying to an ANSWER is accepted, and the CLI resolves the root for you** (measured 2026-09-20, decision-cards round 2). `--reply-to <the answer's event id>` publishes a card tagged `e:<round-1 card> "root"` + `e:<the answer> "reply"`, so both rounds share a thread root and fold into one Asks row. The answer itself carries only `e:<card> "reply"` and no root marker, because its parent IS the root — which is the same asymmetry the rule above describes, seen from the wire. Worth knowing before trusting a fixture: anything grouping cards by thread must read the ROOT, never the parent.
- **A round-2 card does not appear as a timeline row.** It is a reply, so the channel view folds it into round 1's thread; the askee meets it in the Asks inbox or the thread panel. A gate that looks for it in the timeline reads as a product bug and is not one.
- **A card you send yourself is never an ask** — `askForMe` requires author ≠ viewer, with no leniency. So the Asks-inbox half of any card gate needs a SECOND attested identity, and one seat does not have one: the auth tag's signature is over YOUR pubkey (`build_preimage(agent_pubkey, conditions)` in `crates/buzz-sdk/src/nip_oa.rs`), so it cannot be reused for a key you generate, and a fresh key is refused at AUTH. Plan the second author in, or plan the gate out.
